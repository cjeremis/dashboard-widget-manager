...
export function init() {
	...
	$( document ).on( 'change', '.dwm-wizard-line-data-column', function() {
		const selected = [];
		$( '.dwm-wizard-line-data-column:checked' ).each( function() { selected.push( $( this ).val() ); } );
...
export function populateAxes( stepConfig, existingConfig ) {
	...
	stepState.dataColumns = stepState.dataColumns.filter( function( name ) { return validNumericNames.indexOf( normalizeColumnName( name ) ) !== -1; } );
...
export function clearStep() {
	stepState = { labelColumn: '', dataColumns: [], title: '', showLegend: true, availableColumns: [], numericColumns: [] };
	...
	stepState.numericColumns.forEach( function( col, index ) {
		...
		$dataList.append(
			'<label class="dwm-wizard-col-item" for="' + id + '">' +
				'<input type="checkbox" id="' + id + '" class="dwm-wizard-line-data-column" value="' + escapeHtml( normalized ) + '"' + checked + '>' +
