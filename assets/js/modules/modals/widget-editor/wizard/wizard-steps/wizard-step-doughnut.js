...
export function renderStep( $container ) {
	...
	$container.html(
		'<div class="dwm-form-group">' +
			'<select id="dwm-wizard-doughnut-table-select" class="dwm-select dwm-wizard-table-select middle-aligned">' +
				'<option value="">Loading tables…</option>' +
			'</select>' +
		'</div>' +
		'<div id="dwm-wizard-doughnut-columns-grid" class="dwm-wizard-columns-grid" style="display:none">' +
			'<div class="dwm-wizard-columns-header">' +
				...
				'<div class="dwm-wizard-columns-actions">' +
					'<a href="#" class="dwm-wizard-doughnut-select-all">Select All</a>' +
					'<span class="dwm-wizard-columns-separator">|</span>' +
					'<a href="#" class="dwm-wizard-doughnut-deselect-all">Deselect All</a>' +
				'</div>' +
			'</div>' +
			'<div id="dwm-wizard-doughnut-columns-list" class="dwm-wizard-columns-list"></div>' +
		'</div>'
	);
...
export function clearStep() {
	stepState = { table: '', columns: [], availableColumns: [], pendingTableChange: '' };
}
...
function cacheColumns( table, columns ) {
	try {
		sessionStorage.setItem( CACHE_KEY_COLUMNS_PREFIX + table, JSON.stringify( { columns, timestamp: Date.now() } ) );
	} catch ( e ) {}
}
	...
	columns.forEach( function( col ) {
		...
		$list.append(
			'<label class="dwm-wizard-col-item" for="' + checkboxId + '">' +
				'<input type="checkbox" id="' + checkboxId + '" class="dwm-wizard-doughnut-col-checkbox" value="' + colName + '"' + ( isChecked ? ' checked' : '' ) + '>' +
